...
const deleteFavoriteCocktailFromArray = (cocktailId) => {
  const resultFavoriteCocktails = favoriteCocktails.filter(
...
const deleteFavoriteCocktaillFromLocalStorage = (cocktailId) => {
  const favoriteCocktailsFromStorage = JSON.parse(
    localStorage.getItem("dataFavorite")
  );
  const resultFavoriteCocktailsFromStorage =
    favoriteCocktailsFromStorage.filter(
      (cocktail) => cocktail.id !== cocktailId
    );
  localStorage.setItem(
    "dataFavorite",
    JSON.stringify(resultFavoriteCocktailsFromStorage)
  );
};
...
const cocktailIsNotAlreadyFavorite = (cocktailId) => {
  let cocktailFound = favoriteCocktails.find(
    (cocktail) => cocktail.id === cocktailId
  );
...
const addCocktailToFavoriteArray = (cocktailId) => {
  const cocktailFound = cocktails.find(
    (cocktail) => cocktail.id === cocktailId
  );
...
const createFavoriteCocktailFromArray = (cocktailId) => {
  const cocktailFound = favoriteCocktails.find(
    (cocktail) => cocktail.id === cocktailId
  );
  createFavoriteCocktail(
    cocktailFound.id,
    cocktailFound.name,
    cocktailFound.imageUrl
  );
};
